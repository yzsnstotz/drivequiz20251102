...
-- 文件名: 20250122_fix_function_search_path.sql
-- 说明: 为所有函数添加固定的 search_path，防止 SQL 注入攻击
-- 日期: 2025-01-22
...
-- ============================================================
-- 注意: 此函数在 AI 数据库中，需要 ai_vectors 表存在
-- 如果 ai_vectors 表不存在，说明这是主数据库，跳过此函数修复
DO $$
BEGIN
  -- 检查 ai_vectors 表是否存在
  IF EXISTS (
    ...
    IF EXISTS (
      ...
      AND pg_get_function_arguments(p.oid) LIKE '%vector%'
    ) THEN
      -- 向量相似度检索函数（在 AI 数据库中）
      EXECUTE $func$
    ...
    END IF;
  ELSE
    RAISE NOTICE '⚠️  ai_vectors 表不存在（这是主数据库），跳过 match_documents 函数修复';
    RAISE NOTICE '   提示: match_documents 函数应在 AI 数据库中修复，使用 20251111_fix_function_search_path.sql';
  END IF;
END $$;
...
-- ============================================================
-- 12. 修复 sync_question_tags_on_insert 函数
-- ============================================================
