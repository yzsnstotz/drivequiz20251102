    ...
    const loadQuestions = async () => {
      ...
      try {
        ...
        if (pkg?.questionsByLocale && pkg.questionsByLocale[language]) {
          ...
          console.log(`[ExamMode] Using questionsByLocale for language: ${language}, count: ${allQuestions.length}`);
        } else {
          allQuestions = pkg?.questions || [];
          console.log(`[ExamMode] Using default questions, count: ${allQuestions.length}`);
        }
        ...
        <div className="mb-6">
          ...
          {(() => {
            try {
              ...
              console.log('[ExamMode] Image URL validation result:', {
                questionId: currentQuestion.id,
                isValid,
