...
function StudyPage() {
  ...
  if (selectedSet) {
    return <QuestionPage questionSet={selectedSet} onBack={() => setSelectedSet(null)} />;
  }
      ...
      <div className="space-y-8">
        {(dynamicCategories.length > 0 ? dynamicCategories : categories).map((category) => {
          const categoryQuestionSets = questionSets.filter(set =>
            category.questionSets.some(categorySet => categorySet.id === set.id || categorySet.title === set.title)
          );
            ...
            <div key={category.id} className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid gap-4">
                {categoryQuestionSets.map((set) => {
                  const savedProgress = localStorage.getItem(`progress_${set.title}`);
                  const progress = savedProgress ? JSON.parse(savedProgress) : { answeredQuestions: [], totalQuestions: set.totalQuestions };
                  const progressPercentage = Math.round((progress.answeredQuestions?.length / progress.totalQuestions) * 100) || 0;
