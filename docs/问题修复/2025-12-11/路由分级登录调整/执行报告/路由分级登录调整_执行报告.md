...
### 更新记录（追加）
- 调整中间件登录判定：仅使用 `AUTH_REQUIRED_PREFIXES` 决定是否必须登录；删除“非 public 即登录”兜底逻辑；首页与信息页在未登录下可正常访问。
- 再次梳理拦截：移除其他可能的全局登录入口（未发现 withAuth 包裹或跨路由重定向）；`src/middleware.ts` 读取 JWT 登录态，仅当命中 `AUTH_REQUIRED_PREFIXES` 且未登录时跳转登录，其余路径全部放行。
- 自测（本地手动）：未登录访问 `/`、`/license`、`/services`、`/vehicles`、`/about`、`/terms`、`/privacy` 正常；`/study/learn`、`/license/study/xxx`、`/ai`、`/admin` 均跳转登录；已登录访问学习/AI 正常，AI 未激活仍按既有流程；`npm run lint`、`npm run build` 通过（仅存在历史 warning）。
