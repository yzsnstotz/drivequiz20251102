...
1. **框架层对齐：AUTH_URL 同步**
   ...
   - ✅ **自动同步**: v3 已在代码中加入 `AUTH_URL ← NEXTAUTH_URL` 同步逻辑
...
**说明**: 当前生产环境设置了 `NODE_TLS_REJECT_UNAUTHORIZED=0`，这会让所有 HTTPS 请求（包括调用 Google OAuth）不做证书校验，属于**严重安全风险**。

**解决方法**:
1. 在 Vercel Dashboard > Settings > Environment Variables 中
2. **删除** `NODE_TLS_REJECT_UNAUTHORIZED` 环境变量，或
3. 至少**不要在生产环境设置为 0**
