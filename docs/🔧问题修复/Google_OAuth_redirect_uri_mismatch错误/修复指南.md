...
1. **代码精简**
   - ✅ **删除冗余**: 移除所有手动配置的 `redirectUri`，统一使用 `getAuthBaseUrl()` 生成
   - ✅ **精简日志**: env.ts 和 auth.ts 中只保留必要的日志（不超过 3 条）
   - ✅ **统一逻辑**: 所有 redirect_uri 计算只通过 `getAuthBaseUrl()` + `/api/auth/callback/{provider}` 一条路径

2. **TLS 安全强校验**
   - ✅ **fail-fast**: 生产环境检测到 `NODE_TLS_REJECT_UNAUTHORIZED=0` 时，直接阻止应用启动
   - ✅ **代码清理**: 删除所有主动设置 `NODE_TLS_REJECT_UNAUTHORIZED` 的代码

3. **诊断接口精简**
   - ✅ **精简返回**: debug 接口只返回 `NODE_ENV`、`baseUrl`、`expectedRedirectUri` 和 `note`
   - ✅ **唯一真相来源**: 所有 redirect_uri 相关问题，优先检查 debug 接口返回的 `expectedRedirectUri`
...
1. **框架层对齐：AUTH_URL 同步**
   - ✅ **新增**: Auth.js v5 推荐使用 `AUTH_URL`，而此前仅配置了 `NEXTAUTH_URL`
   - ✅ **自动同步**: v3 已在代码中加入双向同步逻辑（`AUTH_URL ↔ NEXTAUTH_URL`）
   - ✅ **统一使用**: 确保 Auth.js 内部使用与 `NEXTAUTH_URL` 一致的 base URL

2. **自诊断接口**
   - ✅ **新增**: `GET /api/auth/debug/google-redirect` 诊断接口
   - ✅ **用途**: 返回服务器视角的 `expectedRedirectUri`，便于对照 Google 控制台配置
   - ✅ **访问**: `https://your-domain.vercel.app/api/auth/debug/google-redirect`

3. **增强日志输出**
   - ✅ **环境变量快照**: 输出 `NEXTAUTH_URL`、`AUTH_URL`、`AUTH_TRUST_HOST` 等关键变量
   - ✅ **预期 redirect_uri**: 明确输出 Google Provider 预期的回调 URI
...
**重要**: 生产环境禁止设置 `NODE_TLS_REJECT_UNAUTHORIZED=0`

**v4 变更**: 自 v4 起，如果生产环境检测到 `NODE_TLS_REJECT_UNAUTHORIZED=0`，应用将**直接启动失败**（fail-fast），而不是只输出警告。
...
**解决方法**:
1. 在 Vercel Dashboard > Settings > Environment Variables 中
2. **删除** `NODE_TLS_REJECT_UNAUTHORIZED` 环境变量（如果存在）
3. 重新部署应用

**说明**: `NODE_TLS_REJECT_UNAUTHORIZED=0` 会让所有 HTTPS 请求（包括调用 Google OAuth）不做证书校验，属于**严重安全风险**。v4 通过 fail-fast 机制强制要求在生产环境中移除此配置。
...
2. 查看返回的 JSON：
   ...
   ```
3. 复制 `expectedRedirectUri` 的值
4. 确保 Google Cloud Console 中配置了完全相同的回调 URI

### 步骤 2：查看 Vercel 部署日志（备选）
...
**如果看到错误**：
- 说明 Vercel 环境变量未配置或配置错误，需要执行下面的步骤 1
