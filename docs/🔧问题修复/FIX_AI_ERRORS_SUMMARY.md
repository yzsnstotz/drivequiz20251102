...
- 修复：
  ...
  - JSON 兜底：`/api/ai/log` 将 JSON 字段统一置 null，`insertAiLog` normalize 防止 “invalid input syntax for type json”；context_tag 统一写 null；无登录态直调 `/api/ai/log` 已实测 insertedId=725/726/727（chat/study_chat/exam_chat）。
