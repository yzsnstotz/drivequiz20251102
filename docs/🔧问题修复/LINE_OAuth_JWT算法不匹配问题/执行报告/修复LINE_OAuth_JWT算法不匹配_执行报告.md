# 修复 LINE OAuth JWT 算法不匹配问题 - 执行报告

**执行日期**: 2025-11-26  
**任务ID**: LINE-OAUTH-FIX-20251126-001  
**当前版本号**: 2025-11-26 06:36:02
...
### 问题描述
LINE OAuth 登录时，NextAuth 验证 ID token 失败，错误信息：`unexpected JWT "alg" header parameter`。LINE 的 `id_token` 使用 HS256 算法签名，但 NextAuth 默认期望 RS256 算法，导致验证失败。

### 修复方案
在 LINE Provider 配置中添加 `client.id_token_signed_response_alg: "HS256"`，告诉 NextAuth/oauth4webapi 使用 HS256 算法验证 LINE 的 `id_token`。

### 修复结果
✅ 已添加 `client.id_token_signed_response_alg: "HS256"` 配置  
✅ 已恢复 token 响应中的 `id_token` 返回  
✅ 已更新相关注释说明
...
### 1. `src/lib/providers/line.ts`

**修改内容**：
1. 添加 `client` 配置，设置 `id_token_signed_response_alg: "HS256"`
2. 恢复 token 响应中的 `id_token` 返回（之前被删除以避免验证）
3. 更新相关注释，说明现在 NextAuth 会使用 HS256 算法验证

**关键修改**：

```typescript
// 添加 client 配置（行 179-183）
client: {
  id_token_signed_response_alg: "HS256",
},

// 恢复 id_token 返回（行 103-120）
return {
  access_token: tokens.access_token,
  expires_in: tokens.expires_in,
  id_token: tokens.id_token, // 现在可以返回 id_token，NextAuth 会使用 HS256 验证
...
**修复方案**：
- 在 LINE Provider 配置中添加 `client.id_token_signed_response_alg: "HS256"`
- 这个配置告诉 `oauth4webapi` 使用 HS256 算法验证 `id_token`
- 现在可以安全地返回 `id_token`，NextAuth 会使用正确的算法验证它
...
return {
  access_token: tokens.access_token,
  expires_in: tokens.expires_in,
  id_token: tokens.id_token, // 现在可以返回，NextAuth 会使用 HS256 验证
  refresh_token: tokens.refresh_token,
  scope: tokens.scope,
  token_type: tokens.token_type,
};
...
1. ✅ **添加 client 配置**
   - 在 LINE Provider 配置中添加 `client.id_token_signed_response_alg: "HS256"`
   - 告诉 NextAuth 使用 HS256 算法验证 LINE 的 `id_token`

2. ✅ **恢复 id_token 返回**
   - 在 token 响应中恢复 `id_token` 返回
   - 现在可以安全地返回 `id_token`，NextAuth 会使用正确的算法验证

3. ✅ **更新注释**
   - 更新相关注释，说明配置的作用和原理
   - 移除了之前关于删除 `id_token` 的注释

4. ✅ **更新版本号**
   - 版本号：`2025-11-26 06:36:02`
