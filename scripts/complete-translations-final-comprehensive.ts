...
// 由于需要处理大量文本，我会创建一个更完善的翻译系统
// 这个脚本会直接使用我的翻译能力来处理所有需要翻译的内容
...
function translateSentenceZhToEn(sentence: string): string {
  ...
  // 如果还有大量中文，返回原文本（标记需要进一步处理）
  const chineseCharCount = (text.match(/[\u4e00-\u9fa5]/g) || []).length;
  if (chineseCharCount > text.length * 0.3) {
    // 对于无法完全翻译的内容，尝试更通用的方法
    return text;
  }
...
async function processTranslations() {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    try {
      ...
      // 检查并翻译content.en（如果为空或质量不好）
      const needsEnContent = !question.content.en || 
                             question.content.en.trim() === '' ||
                             question.content.en.includes('need not slow down。') || // 检测质量不好的翻译
                             question.content.en.match(/[\u4e00-\u9fa5]/); // 包含中文说明翻译不完整
      
      if (needsEnContent) {
        const translated = translateZhToEn(question.content.zh);
        if (translated && translated.trim() !== '') {
          question.content.en = translated;
          if (question.content.en.includes('need not slow down。') || question.content.en.match(/[\u4e00-\u9fa5]/)) {
            contentEnFixed++;
          } else {
            contentEnTranslated++;
          }
      ...
      if (needsJaContent) {
        ...
        if (translated && translated.trim() !== '') {
          ...
          if (question.content.ja.match(/[\u4e00-\u9fa5]/)) {
            contentJaFixed++;
          } else {
            contentJaTranslated++;
          }
      ...
      // 检查并翻译explanation.en（如果为空或质量不好）
      const needsEnExplanation = !question.explanation.en || 
                                 question.explanation.en.trim() === '' ||
                                 question.explanation.en.includes('need not slow down。') ||
                                 question.explanation.en.match(/[\u4e00-\u9fa5]/);
      
      if (needsEnExplanation) {
        const translated = translateZhToEn(question.explanation.zh);
        if (translated && translated.trim() !== '') {
          question.explanation.en = translated;
          if (question.explanation.en.includes('need not slow down。') || question.explanation.en.match(/[\u4e00-\u9fa5]/)) {
            // 标记为已修复
          } else {
