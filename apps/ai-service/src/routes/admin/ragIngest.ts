// apps/ai-service/src/routes/admin/ragIngest.ts
/**
 * ZALEM · AI-Service
 * Admin: RAG Document Ingestion API
 *
 * POST /v1/admin/rag/ingest
 * - 服务侧鉴权：Authorization: Bearer <SERVICE_TOKEN>
 * - 接收文档内容，执行：分片→嵌入→批量写 ai_vectors → 统计 chunks
...
async function createEmbedding(
  ...
  try {
    const resp = await openai.embeddings.create({
      ...
      input: text.slice(0, 8000), // OpenAI 限制
    });
